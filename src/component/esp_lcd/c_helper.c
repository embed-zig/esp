...
static esp_err_t parse_data_endian(uint32_t value, lcd_rgb_data_endian_t *out)
{
    if (out == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
    ...
    switch (value) {
    case 0:
        *out = LCD_RGB_DATA_ENDIAN_BIG;
        return ESP_OK;
    case 1:
        *out = LCD_RGB_DATA_ENDIAN_LITTLE;
        return ESP_OK;
    default:
        return ESP_ERR_INVALID_ARG;
    }
        ...
        esp_lcd_panel_handle_t *))
{
    ...
    lcd_rgb_data_endian_t endian = LCD_RGB_DATA_ENDIAN_BIG;
    const esp_err_t endian_err = parse_data_endian(data_endian, &endian);
    if (endian_err != ESP_OK) {
        return endian_err;
    }
